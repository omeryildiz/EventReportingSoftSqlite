...
public class DBConnection extends javax.swing.JFrame {
    ...
    private final String url = "jdbc:sqlite";//veritabanının adresi ve portu
    private final String dbName = ":../db/etkinlikler.sqlite"; //veritabanının ismi
    private final String driver = "org.sqlite.JDBC";//mySQL ile Java arasındaki bağlantıyı sağlayan JDBC sürücüsü
    ...
    public Statement baglantiAc() throws Exception {
        ...
        conn = DriverManager.getConnection(url + dbName);//bağlantı açılıyor
    ...
    public void TabloDoldur() {
        try {
            ...
            Statement st = baglantiAc(); //Bağlantıyı aç         
            ResultSet res = st.executeQuery("SELECT * FROM sks_tablo"); //VT'den kayıtları ResultSet'e al
            myTableModel model = new myTableModel(res); //Tablomuza model oluştur
            jTable1.setModel(model); //Tabloyu res'teki kayıtlar ile doldur          
            baglantiKapat(); //Bağlantıyı kapat
    ...
    public void KayitSil(int ID) {
        try {
            Statement st = baglantiAc();
            st.executeUpdate("Delete FROM sks_tablo where ID=" + String.valueOf(ID));
            baglantiKapat(); //bağlantıyı kapat
            TabloDoldur();  //Penceremizdeki tabloyu yeniden doldur (güncelle)
    ...
    public void KayitGuncelle(String ID, String ad,String soyad,String cinsiyet,String yer,String katilimci,String konusmaci) {
        try {
            ...
            Statement st = baglantiAc();
            String sql = "UPDATE sks_tablo SET " +
    "isim='" + ad + "', " +
    "aciklama='" + soyad + "', " +
    "yer='" + yer + "', " +
    "faaliyet=" + cinsiyet +", " +
	...
	AdTextField.setText(jTable1.getValueAt(jTable1.getSelectedRow(), 1).toString());
        ...
        else if ( SwingUtilities.isRightMouseButton( evt ))
		{
			...
			model.setSelectionInterval( rowNumber, rowNumber );
                        ...
                        menuItemRemove = new JMenuItem("Sil");
                        final String a = jTable1.getValueAt(jTable1.getSelectedRow(), 0).toString();
                        menuItemRemove.addActionListener(new java.awt.event.ActionListener(){
                            
                            public void actionPerformed(java.awt.event.ActionEvent evt) {                         
                            menuItemActionPerformed(evt,a);
        }   
                        });
                       ...
                       /* Başarılı
                        JMenuItem menu = new JMenuItem("Klienci");                        
                        JPopupMenu popup = new JPopupMenu();
                        popup.add(menu);
                        popup.show(evt.getComponent(), evt.getX(), evt.getY());
                        */
    ...
    private void menuItemActionPerformed(ActionEvent evt,String a) {
        ...
        if (menu == menuItemRemove) {
            try {
                ...
                JOptionPane.showConfirmDialog(null, "silinemedi", "SQLite Bağlantısında Hata", JOptionPane.PLAIN_MESSAGE);
        }
